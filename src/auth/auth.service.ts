...
import type { AccessTokenDto } from './dto/jwt.dto';
import type { userDataFromProvider } from '../user/interfaces/userDataFromProvider.interface';
    ...
    async validateUserWithEmailPassword(
        ...
        if (user.provider)
            throw new UnauthorizedException(
                `User can't login with Username and Password, already has account on ${user.provider}`
            );
